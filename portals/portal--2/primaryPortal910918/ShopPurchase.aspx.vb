...
Partial Public Class ShopPurchase
    ...
    Protected Sub LinkButton1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles LinkButton1.Click
        ...
        Try
            ref = New localhost.SpringWSEndPointService
            '******* setup proxy for send transaction by proxy here ********
            '*******setup bypass proxy in web.xml 
            'IP your system must be domain your IP proxy server

            'Dim myproxy As IWebProxy = New WebProxy("172.17.18.14", 8080)
            ' myproxy.Credentials = New NetworkCredential("M.Jazimagh", "1234")
            'myproxy.Credentials = New NetworkCredential("r.ashrafi", "raha_2013")
            'ref.Proxy = myproxy
            'ref.UseDefaultCredentials = False
            '***********************************************************
            Dim credCache As CredentialCache = New CredentialCache()
            Dim netCred As NetworkCredential = New NetworkCredential("Merchant", "test")
